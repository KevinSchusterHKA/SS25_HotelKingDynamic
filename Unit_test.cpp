...
void UNITTEST_cpu() {
	...
	for (size_t i = 0; i < amount; ++i) {
		if (players[i].getHuman() == CPU1) {
			...
			for (int build = 0; build < 25; ++build) {
				...
				std::cout << "try " << build + 1 << ": CPU(ID)" << players[i].getID()
					<< (built ? " built a house." : " did not build a house.") << std::endl;
			}

			//street buy
			bool bought = players[i].tryBuyStreetcpu(map);
			std::cout << "CPU(ID)" << players[i].getID() << (bought ? " bought a street." : " did not buy a street.") << std::endl;
			
			//Handel cpu to player
			int targetPlayer = -1;
			int propertyIndex = -1;
			int offer = players[i].handelcpu(players[i].getID(), amount, playerRefs, targetPlayer, propertyIndex, map);
			if (offer != -1 && targetPlayer != -1 && propertyIndex != -1) {
				std::cout << "CPU with id" << players[i].getID() << " offers " << offer << " for property "
					<< propertyIndex << " from player(with id) " << players[targetPlayer].getID() << std::endl;
				//player to cpu 
				if (players[targetPlayer].getHuman() == CPU1) {
					bool accepted = players[i].acceptTradecpu(propertyIndex, offer, i, playerRefs,map);
					std::cout << "CPU(ID)" << players[targetPlayer].getID() << (accepted ? " accepted the trade." : " rejected the trade.") << std::endl;
				}
				else {
					std::cout << "Human player(ID) " << players[targetPlayer].getID() << " needs to decide on the offer." << std::endl;
				}
			}
			else {
				std::cout << "CPU(ID)" << players[i].getID() << " did not make a trade offer." << std::endl;
			}
	...
	for (size_t i = 0; i < amount; ++i) {
		if (players[i].getHuman() == CPU1) {
			...
			std::cout << "CPU(ID)" << players[i].getID() << (willTakeBahn ? " decided to take the Bahn." : " decided NOT to take the Bahn.") << std::endl;
		}
	}
	players[2].setBudget(50);  
	players[2].addStrasse(16);  
	players[2].baueHausTEMP(16, map);  
	std::cout << "Before Verkauf: Budget = $" << players[2].getBudget() << "\n";
	players[2].cpuHausOderStrassenVerkauf(map);
	std::cout << "After Verkauf: Budget = $" << players[2].getBudget() << "\n";
}
